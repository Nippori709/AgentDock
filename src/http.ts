...
import { expandHome, loadConfig, type LocalWorkspaceBridgeConfig } from "./config.js";
import {
  profilePathForRoot,
...
const WRITE_MODES = ["workspace", "off"] as const;
...
function runtimeTunnelFallback(): TunnelMode {
  if (process.env.LOCALWORKSPACEBRIDGE_TUNNEL && TUNNELS.includes(process.env.LOCALWORKSPACEBRIDGE_TUNNEL as TunnelMode)) {
    return process.env.LOCALWORKSPACEBRIDGE_TUNNEL as TunnelMode;
  }
  return process.env.LOCALWORKSPACEBRIDGE_TUNNEL_MODE === "0" ? "none" : "cloudflare";
}
...
function profileValues(config: LocalWorkspaceBridgeConfig, profile = readWorkspaceProfile(config.defaultRoot)): ProfileFormValues {
  const hostname =
    profile.hostname ??
    ...
    "";
  const mode = oneOf(profile.mode ?? process.env.LOCALWORKSPACEBRIDGE_MODE, MODES, "agent");
...
function serverUrlDisplay(endpoint: string | undefined, authEnabled: boolean, oauthEnabled = false): string {
  if (!endpoint) return "";
  const safeEndpoint = redactSensitiveText(endpoint);
  return oauthEnabled ? `${safeEndpoint} (OAuth)` : authEnabled ? `${safeEndpoint} (Authorization: Bearer required)` : safeEndpoint;
}
...
function currentTunnelMessage(tunnel: TunnelMode, endpoint: string): string {
  ...
  if (tunnel === "ngrok") return "Enter your reserved ngrok domain, or set NGROK_DOMAIN before starting LocalWorkspaceBridge.";
...
function profileForm(config: LocalWorkspaceBridgeConfig): string {
  ...
  const runtimeUrl = serverUrlDisplay(runtimeEndpoint, Boolean(config.authToken), Boolean(config.publicUrl));
  const savedEndpoint = values.hostname ? `https://${values.hostname}/mcp` : "";
  const savedUrl = serverUrlDisplay(savedEndpoint, Boolean(config.authToken), Boolean(config.publicUrl));
  ...
  return `<section class="panel profile-panel" id="profile">
      ...
      <form class="profile-form" data-profile-form>
        ...
        <p class="note" data-profile-status>Tokens stay hidden. Restart LocalWorkspaceBridge for saved profile changes to apply.</p>
      </form>
    </section>`;
}

function buildProfilePayload(config: LocalWorkspaceBridgeConfig, existing: WorkspaceProfile, input: AdminProfilePatch): WorkspaceProfile {
...
function profileResponse(config: LocalWorkspaceBridgeConfig): Record<string, unknown> {
  const profile = readWorkspaceProfile(config.defaultRoot);
  const runtime = readRuntimeConnection(config.defaultRoot);
...
Usage:
  local-workspace-bridge-mcp-http --root /path/to/repo --port 8787
  local-workspace-bridge-mcp-http --version
  local-workspace-bridge-mcp-http --help

Set LOCALWORKSPACEBRIDGE_HTTP_TOKEN for public/tunnel use.
For trusted local-only testing, set LOCALWORKSPACEBRIDGE_ALLOW_NO_HTTP_TOKEN=1.
Most users should run: local-workspace-bridge start`);
}

function onboardingPage(config: LocalWorkspaceBridgeConfig): string {
  const localMcp = `http://${config.host}:${config.port}/mcp`;
  const localMcpDisplay = config.authToken ? `${localMcp} (Authorization: Bearer required)` : localMcp;
  const allowedRoots = config.allowedRoots.map((root) => `<li>${escapeHtml(root)}</li>`).join("");
  const authLabel = config.publicUrl && config.authToken ? "OAuth + PKCE" : config.authToken ? "Token protected" : "Disabled";
  ...
  const controls = [
    copyCommand("Re-run setup wizard", "Use the CLI for broader profile edits that are intentionally not exposed here.", "local-workspace-bridge setup"),
    ...
    copyCommand("Start without bash", "Restart with file tools but no ChatGPT-triggered bash tool.", `local-workspace-bridge start --root ${rootArg} --no-bash`),
    copyCommand("Require explicit bash target", "Restart so bash calls must include this matching session_id.", `local-workspace-bridge start --root ${rootArg} --bash-session ${sessionArg} --require-bash-session`),
    copyCommand("Show Codex session list", "Restart with read-only local Codex session metadata in full tool mode.", `local-workspace-bridge start --root ${rootArg} --tool-mode full --codex-sessions metadata`),
    copyCommand("Read Codex transcripts", "Restart with bounded local transcript reads from Codex JSONL history.", `local-workspace-bridge start --root ${rootArg} --tool-mode full --codex-sessions read`),
    copyCommand("Use full bash transcript", "Restart with the raw stdout/stderr transcript instead of compact tool cards.", `local-workspace-bridge start --root ${rootArg} --bash-transcript full`)
  ].join("");
  return `<!doctype html>
<html lang="en">
<head>
  ...
  <title>LocalWorkspaceBridge Local Control - ChatGPT Workspace Agent</title>
  <style>
    :root {
      color-scheme: light;
...
<body>
  <main>
    <header class="topbar">
      <div class="brand">
        ...
        <span>
          ...
          <span class="brand-title">LocalWorkspaceBridge</span>
        </span>
      </div>
      <nav class="quick-links" aria-label="LocalWorkspaceBridge resources">
      ...
      <aside class="side-stack">
        <section class="panel guide-panel" id="guide">
          ...
          <div class="guide-list">
            ...
            <div class="guide-item"><span class="num">3</span><span><strong>Create the ChatGPT app</strong><p>Choose Server URL, paste the stable credential-free URL, select OAuth, and enter the local approval key on the consent page.</p></span></div>
            <div class="guide-item"><span class="num">4</span><span><strong>Restart for policy changes</strong><p>Saved profile changes apply when LocalWorkspaceBridge starts again. The live server does not mutate under an active ChatGPT session.</p></span></div>
          </div>
        </section>
    ...
    <section class="workspace-grid">
      <section class="panel" id="connect">
        ...
        <ol class="steps">
          ...
          <li><span class="num">3</span><span>Paste the public LocalWorkspaceBridge URL from the terminal.</span></li>
          <li><span class="num">4</span><span>Select <code>OAuth</code> and complete the LocalWorkspaceBridge consent page.</span></li>
        </ol>
    ...
    <details class="panel details-panel">
      ...
      <p class="note">LocalWorkspaceBridge rejects workspace access outside these roots.</p>
    </details>
  ...
  <script>
    ...
    function serverPreviewFor(hostname) {
      ...
      if (!clean) return "";
      return "https://" + clean + "/mcp" + (tokenEnabled ? " (OAuth)" : "");
    }
    ...
    if (profileForm) {
      profileForm.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          if (status) status.textContent = "Saved. Restart LocalWorkspaceBridge for these profile settings to apply.";
        } catch (error) {
...
async function main(): Promise<void> {
  ...
  if (config.requireHttpToken && !config.authToken) {
    throw new Error(
      "LOCALWORKSPACEBRIDGE_HTTP_TOKEN is required for this HTTP binding. " +
        "Set LOCALWORKSPACEBRIDGE_HTTP_TOKEN, use `local-workspace-bridge start` to generate one, " +
        "or set LOCALWORKSPACEBRIDGE_ALLOW_NO_HTTP_TOKEN=1 only for a trusted local-only setup."
    );
  }
  ...
  const modernMcpCompatHandler = {
    fetch: async (request: globalThis.Request, options?: { parsedBody?: unknown }) => {
      const response = await modernMcpHandler.fetch(request, options);
      const body = options?.parsedBody as { method?: unknown } | undefined;
      return body?.method === "tools/list" ? patchModernToolSecuritySchemes(config, response) : response;
    }
  };
  const handleModernMcp = toNodeHandler(modernMcpCompatHandler, {
    onerror: (error) => console.error(`[LocalWorkspaceBridge] modern MCP handler: ${redactSensitiveText(error.message)}`)
  });

  function isModernMcpRequest(body: unknown): boolean {
    if (!body || typeof body !== "object" || Array.isArray(body)) return false;
    const request = body as { method?: unknown; params?: { _meta?: Record<string, unknown> } };
    if (request.method === "server/discover") return true;
    return request.params?._meta?.["io.modelcontextprotocol/protocolVersion"] === "2026-07-28";
  }
  ...
  app.use((req, res, next) => {
    ...
    (res as any).write = (chunk: unknown, ...args: unknown[]) => {
      if (typeof chunk === "string" || Buffer.isBuffer(chunk) || chunk instanceof Uint8Array) responseBytes += Buffer.byteLength(chunk as any);
    ...
    console.error(`[LocalWorkspaceBridge] ${req.method} ${req.path} received`);
    res.on("finish", () => {
      const body = req.body as { id?: unknown; method?: unknown; params?: { name?: unknown; arguments?: { cursor?: unknown } } } | undefined;
      const cursor = body?.params?.arguments?.cursor;
      const details = req.path === "/mcp" ? {
        request_id: body?.id ?? null,
        method: body?.method ?? null,
        tool_name: body?.params?.name ?? null,
        cursor_hash: typeof cursor === "string" && cursor ? createHash("sha256").update(cursor).digest("hex").slice(0, 12) : null,
        response_bytes: responseBytes
      } : { response_bytes: responseBytes };
      console.error(`[LocalWorkspaceBridge] ${req.method} ${req.path} -> ${res.statusCode} ${Date.now() - started}ms ${JSON.stringify(redactStructured(details))}`);
    });
    res.on("close", () => {
      if (!res.writableEnded) console.error(`[LocalWorkspaceBridge] ${req.method} ${req.path} connection_closed ${Date.now() - started}ms response_bytes=${responseBytes}`);
    });
    next();
  });
  ...
  app.use((req, res, next) => {
    ...
    if (oauth && req.path === "/mcp") {
      next();
      return;
    }
    const bearer = req.headers.authorization?.match(/^Bearer\s+(.+)$/i)?.[1]?.trim();
    const queryToken = config.allowQueryToken
      ? typeof req.query.local_workspace_bridge_token === "string"
        ? req.query.local_workspace_bridge_token
  ...
  app.post("/admin/profile", adminRateLimit, adminBodyLimit, express.json({ limit: "32kb" }), (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        message: "Saved. Restart LocalWorkspaceBridge for these profile settings to apply."
      });
    } catch (error) {
  ...
  app.post("/mcp", express.json({ limit: "20mb" }), async (req, res) => {
    try {
      if (isModernMcpRequest(req.body)) {
        await handleModernMcp(req, res, req.body);
        return;
      }
      ...
      if (!res.headersSent) {
        res.status(500).json({
          jsonrpc: "2.0",
          error: { code: -32603, message: "Internal LocalWorkspaceBridge MCP error. Check the local terminal for details." },
          id: null
        });
      }
    }
  ...
  app.listen(config.port, config.host, () => {
    console.error(`[LocalWorkspaceBridge] HTTP MCP listening on http://${config.host}:${config.port}/mcp`);
    console.error(`[LocalWorkspaceBridge] defaultRoot=${config.defaultRoot}`);
    console.error(`[LocalWorkspaceBridge] allowedRoots=${config.allowedRoots.join(", ")}`);
    console.error(`[LocalWorkspaceBridge] bashMode=${config.bashMode}`);
    console.error(`[LocalWorkspaceBridge] writeMode=${config.writeMode}`);
    console.error(`[LocalWorkspaceBridge] widgetDomain=${config.widgetDomain}`);
    console.error(`[LocalWorkspaceBridge] oauth=${oauth ? `enabled issuer=${config.publicUrl}` : "disabled"}`);
  });
}
