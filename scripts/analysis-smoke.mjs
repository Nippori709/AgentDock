...
const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'local-workspace-bridge-analysis-'));
const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'local-workspace-bridge-analysis-outside-'));
...
try {
  await write('package.json', JSON.stringify({ name: 'fixture', packageManager: 'pnpm@9.15.0', main: 'dist/stdio.js', bin: { fixture: 'bin/cli.mjs' }, scripts: { test: 'node --test', dev: 'tsx src/dev.ts' } }, null, 2));
  await write('bin/cli.mjs', 'console.log("fixture");\n');
  await write('bin/Debug/generated.js', 'throw new Error("generated output should be ignored");\n');
  await write('src/dev.ts', 'export const dev = true;\n');
  await write('src/stdio.ts', 'export const stdioEntrypoint = true;\n');
  await write('pyproject.toml', '[project]\nname = "fixture"\n\n[project.scripts]\nfixture-py = "python.cli:main"\n');
  await write('python/cli.py', 'def main():\n    return 0\n');
  await write('.gitignore', 'ignored-vendor/\nout/*\n!out/keep.ts\n');
  await write('.venv/Lib/site-packages/_pytest/main.py', 'def dependency_entry():\n    return True\n');
  await write('ignored-vendor/main.py', 'def ignored_entry():\n    return True\n');
  await write('out/keep.ts', 'export const kept = true;\n');
  await write('out/drop.ts', 'export const dropped = true;\n');
  ...
  await write('scripts/analysis-smoke.mjs', "import '../src/auth.js';\n");
  await write('scripts/api-integration.mjs', "import '../src/index.js';\n");
  await write('scripts/browser-e2e.mjs', "import '../src/index.js';\n");
  await write('scripts/smoke.mjs', "import '../src/index.js';\n");
  ...
  const lowMemoryConfig = loadConfig(['--root', tmp, '--low-memory', '--bash', 'off', '--write', 'off']);
  assert.equal(lowMemoryConfig.lowMemory, true);
  assert.equal(lowMemoryConfig.maxSearchResults, 100);
  assert.equal(lowMemoryConfig.maxHttpSessions, 16);
  assert.equal(lowMemoryConfig.httpSessionTtlMs, 10 * 60_000);
  assert.equal(lowMemoryConfig.analysisCacheEntries, 2);
  assert.equal(lowMemoryConfig.analysisCacheTtlMs, 5 * 60_000);
  assert.equal(lowMemoryConfig.analysisLimits.maxInventoryFiles, 5_000);
  assert.equal(lowMemoryConfig.analysisLimits.maxAnalyzedFiles, 800);
  assert.equal(lowMemoryConfig.analysisLimits.maxScannedBytes, 16 * 1024 * 1024);
  assert.equal(lowMemoryConfig.analysisLimits.maxSymbols, 20_000);
  assert.equal(lowMemoryConfig.analysisLimits.maxRelationships, 40_000);
  ...
  for (const testPath of ['scripts/analysis-smoke.mjs', 'scripts/api-integration.mjs', 'scripts/browser-e2e.mjs', 'scripts/smoke.mjs']) {
    assert(result.files.some((file) => file.path === testPath && file.role === 'test'), `${testPath} was not classified as a test`);
  }
  assert(!result.files.some((file) => file.path === '.env'));
  assert(classification.detectProjectTypes(result.files).includes('node'));
  assert(result.files.some((file) => file.path === 'bin/cli.mjs' && file.entrypoint === true));
  assert(!result.files.some((file) => file.path.startsWith('bin/Debug/')));
  assert(result.files.some((file) => file.path === 'src/dev.ts' && file.entrypoint === true));
  assert(result.files.some((file) => file.path === 'src/stdio.ts' && file.entrypoint === true));
  assert(result.files.some((file) => file.path === 'python/cli.py' && file.entrypoint === true));
  assert(!result.files.some((file) => file.path.startsWith('.venv/')));
  assert(!result.files.some((file) => file.path.startsWith('ignored-vendor/')));
  assert(result.files.some((file) => file.path === 'out/keep.ts'));
  assert(!result.files.some((file) => file.path === 'out/drop.ts'));
