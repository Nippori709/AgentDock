...
const cli = path.join(projectRoot, 'scripts', 'local-workspace-bridge.mjs');
const root = await fs.mkdtemp(path.join(os.tmpdir(), 'local-workspace-bridge-analysis-cli-'));
const home = await fs.mkdtemp(path.join(os.tmpdir(), 'local-workspace-bridge-analysis-cli-home-'));
...
function run(args) {
  return spawnSync(process.execPath, [cli, ...args], {
    ...
    encoding: 'utf8',
    timeout: 5000,
    env: { ...process.env, NO_COLOR: '1', CI: '1', LOCALWORKSPACEBRIDGE_HOME: home }
  });
}
...
try {
  ...
  const nonGitRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'local-workspace-bridge-analysis-cli-non-git-'));
  try {
