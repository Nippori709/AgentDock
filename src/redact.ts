...
const AUTHORIZATION_FIELD_PATTERN = /(["']?Authorization["']?\s*:\s*["']?Bearer\s+)[A-Za-z0-9._~+/=-]{8,}/gi;
...
const QUERY_TOKEN_PATTERN = /([?&](?:local_workspace_bridge_token|token|access_token|auth_token|api[_-]?key)=)[^&\s"'`<>]{8,}/gi;
const LOCALWORKSPACEBRIDGE_TOKEN_ASSIGNMENT_PATTERN = /\b(local_workspace_bridge_token\s*=\s*)(?:"[^"\r\n]{8,512}"|'[^'\r\n]{8,512}'|`[^`\r\n]{8,512}`|[A-Za-z0-9_./+=-]{8,512})/gi;
const LOCALWORKSPACEBRIDGE_TOKEN_FIELD_PATTERN = /(["']?local_workspace_bridge_token["']?\s*:\s*)(?:"[^"\r\n]{8,512}"|'[^'\r\n]{8,512}'|`[^`\r\n]{8,512}`|[A-Za-z0-9_./+=-]{8,512})/gi;
...
const SECRET_PATTERNS = [OPENAI_SECRET_PATTERN, COMMON_TOKEN_PATTERN, BEARER_TOKEN_PATTERN, AUTHORIZATION_FIELD_PATTERN, CLI_TOKEN_PATTERN, QUERY_TOKEN_PATTERN, LOCALWORKSPACEBRIDGE_TOKEN_ASSIGNMENT_PATTERN, LOCALWORKSPACEBRIDGE_TOKEN_FIELD_PATTERN, SECRET_ASSIGNMENT_PATTERN, SECRET_FIELD_PATTERN];
...
export function redactSensitiveText(text: string): string {
  return text
    .replace(LOCALWORKSPACEBRIDGE_TOKEN_ASSIGNMENT_PATTERN, (_match, prefix) => `${prefix}[REDACTED_SECRET]`)
    .replace(LOCALWORKSPACEBRIDGE_TOKEN_FIELD_PATTERN, (_match, prefix) => `${prefix}[REDACTED_SECRET]`)
    ...
    .replace(AUTHORIZATION_FIELD_PATTERN, (_match, prefix) => `${prefix}[REDACTED_SECRET]`)
...
function isPlaceholderSecret(value: string): boolean {
  ...
  return (
    ...
    normalized.includes("keep-this-local-workspace-bridge-token-stable") ||
