...
export type AnalysisMode = "inventory_only" | "symbols" | "relationships" | "full";

function cacheKey(workspace: Workspace, fingerprint: string, config: LocalWorkspaceBridgeConfig, mode: AnalysisMode): string {
  return `${workspace.id}:${fingerprint}:${JSON.stringify({ limits: config.analysisLimits, ignore: config.analysisIgnoreGlobs, mode })}`;
}
...
export async function inspectWorkspace(
  ...
  if (!config.analysisEnabled) throw new Error("Repository analysis is disabled by LOCALWORKSPACEBRIDGE_ANALYSIS=0.");
  const mode = options.mode ?? "full";
  const inventory = await inventoryWorkspace(config, guard, workspace);
  const key = cacheKey(workspace, inventory.fingerprint, config, mode);
  const cacheOptions = { maxEntries: config.analysisCacheEntries, ttlMs: config.analysisCacheTtlMs };
  const cached = getCachedWorkspaceAnalysis(key, cacheOptions);
  if (cached) return { ...cached, cache: { hit: true, key } };

  const extraction = mode === "inventory_only"
    ? { files: [], analyzedFiles: 0, scannedBytes: 0, truncated: false, warnings: [] }
    : await extractWorkspaceFiles(config, guard, workspace, inventory.files);
  const symbols = mode === "inventory_only"
    ? []
    : extraction.files.flatMap((file) => file.symbols).slice(0, config.analysisLimits.maxSymbols);
  const relationships = mode === "relationships" || mode === "full"
    ? buildRelationships(extraction.files, inventory.files, config.analysisLimits.maxRelationships)
    : [];
  ...
  setCachedWorkspaceAnalysis(key, result, cacheOptions);
  return result;
}
...
export async function searchWorkspaceStructured(
  ...
  const analysisMode: AnalysisMode = options.regex
    ? "inventory_only"
    : intent === "references" || intent === "impact"
      ? "full"
      : "symbols";
  const analysis = await inspectWorkspace(config, guard, workspace, { mode: analysisMode });
