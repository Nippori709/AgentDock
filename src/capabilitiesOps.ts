...
export type SkillCapabilitySummary = Pick<SkillInventoryItem, "name" | "source" | "stable_id" | "discovered" | "configured" | "callable" | "healthy" | "permission_required" | "preferred">;

function stableSkillId(source: SkillInventoryItem["source"], name: string): string {
  return `skill_${createHash("sha256").update(`${source}:${name}`).digest("hex").slice(0, 16)}`;
}
...
function publicSkill(record: SkillInventoryRecord): SkillInventoryItem {
  return {
    ...
    path: record.path,
    stable_id: stableSkillId(record.source, record.name),
    discovered: true,
    configured: record.readable,
    callable: false,
    healthy: false,
    permission_required: true,
    reason: record.readable
      ? "Skill instructions can be loaded, but dependent tools are not exposed or health-checked by LocalWorkspaceBridge."
      : "SKILL.md was discovered but could not be read.",
...
async function discoverSkillRecords(
  ...
  for (const file of skillFiles.slice(0, maxSkills)) {
    ...
    items.push({
      ...
      absPath: realFile,
      readable,
      stable_id: stableSkillId(skillSource(realFile, workspace.root), name),
      discovered: true,
      configured: readable,
      callable: false,
      healthy: false,
  ...
  const sorted = unique(items, (item) => `${item.source}:${item.name}:${item.path}`).sort(compareSkills);
...
export async function discoverSkillInventory(
  ...
  const records = await discoverSkillRecords(workspace, options);
  return records.filter((record) => record.preferred).map(publicSkill);
}
...
export async function loadSkill(
  ...
  if (matches.length > 1 && requestedPath) {
  ...
  const skill = matches.find((candidate) => candidate.preferred) ?? matches[0];
...
function parseTomlMcpServers(text: string, source: string): McpServerInventoryItem[] {
  ...
  while ((match = re.exec(text))) {
    out.push({ name: match[2], source, discovered: true, configured: true, callable: false, healthy: false, permission_required: true, reason: "Configured MCP server is inventoried only; LocalWorkspaceBridge does not expose mcp_invoke." });
  }
  return out;
}
...
function parseJsonMcpServers(text: string, source: string): McpServerInventoryItem[] {
  try {
    ...
    return Object.keys(servers).map((name) => ({ name, source, discovered: true as const, configured: true as const, callable: false as const, healthy: false as const, permission_required: true, reason: "Configured MCP server is inventoried only; LocalWorkspaceBridge does not expose mcp_invoke." }));
  } catch {
    return [];
  }
}
...
export async function localWorkspaceBridgeInventory(
  config: LocalWorkspaceBridgeConfig,
  workspace: Workspace,
  options: { includeGlobalSkills?: boolean; includeMcpServers?: boolean; maxSkills?: number; skillCursor?: number; compact?: boolean } = {}
): Promise<{
  text: string;
  skills: Array<SkillInventoryItem | SkillCapabilitySummary>;
  mcpServers: McpServerInventoryItem[];
  nextSkillCursor?: number;
  skillTotalAtLeast: number;
}> {
  const pageSize = Math.max(1, Math.min(options.maxSkills ?? 20, 120));
  const skillCursor = Math.max(0, Math.min(options.skillCursor ?? 0, 499));
  const discovered = await discoverSkillInventory(workspace, {
    includeGlobal: options.includeGlobalSkills !== false,
    maxSkills: Math.min(500, skillCursor + pageSize + 1)
  });
  const page = discovered.slice(skillCursor, skillCursor + pageSize);
  const skills = options.compact === false ? page : page.map((skill): SkillCapabilitySummary => ({
    name: skill.name,
    source: skill.source,
    ...
    permission_required: skill.permission_required,
    preferred: skill.preferred
  }));
  const nextSkillCursor = discovered.length > skillCursor + pageSize ? skillCursor + pageSize : undefined;
  ...
  const skillLines = skills.length
    ? skills.map((skill) => `- ${skill.name} [${skill.source}] callable=${skill.callable} healthy=${skill.healthy}`).join("\n")
    : "- none discovered";
  const mcpLines = mcpServers.length
    ? mcpServers.map((server) => `- ${server.name} (${server.source})`).join("\n")
    : "- none discovered";

  const text = `# LocalWorkspaceBridge Inventory
...
Cursor: ${skillCursor}${nextSkillCursor !== undefined ? `; next: ${nextSkillCursor}` : ""}
...
`;
  ...
  return { text, skills, mcpServers, ...(nextSkillCursor !== undefined ? { nextSkillCursor } : {}), skillTotalAtLeast: discovered.length };
}
