export const TOOL_CARD_URI = "ui://widget/local-workspace-bridge-tool-card.html";
  ...
  <script>
    (() => {
      ...
      function extractStructuredContent(value, depth = 0, seen = new Set()) {
        ...
        if (normalized.local_workspace_bridge_tool || normalized.local_workspace_bridge_title) return normalized;
        const candidates = [
      ...
      function renderGeneric(data) {
        const title = asText(data.local_workspace_bridge_title, "Tool result");
        const preview = JSON.stringify(data, null, 2);
        return card(title, "LocalWorkspaceBridge", "Ready", "good", codeBlock("Result", preview));
      }

      function renderUnavailable() {
        copyableText = "";
        root.innerHTML = card("Result unavailable", "LocalWorkspaceBridge", "Retry", "warn", '<div class="notice">The tool finished, but its display data did not reach this card. Refresh the ChatGPT plugin connection and try the action once more.</div>');
      }

      function renderPending() {
        copyableText = "";
        root.innerHTML = card("Preparing result", "LocalWorkspaceBridge", "Loading", "", '<div class="notice">Loading the tool result…</div>', "pending");
      }
      ...
      function render(data) {
        ...
        copyableText = "";
        const tool = asText(data.local_workspace_bridge_tool, "");
